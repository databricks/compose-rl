...
from compose_rl.algorithms.online.generation_utils.vllm_actor import (
    LLMRayActorAsync, LLMRayActor,
)
...
class DistributedGPUActor(BaseDistributedGPUActor):
    ...
    def sync_weights(self, vllm_engines: list):
        ...
        for name, param in self.model.named_parameters():
            # vllm_engine.update_weight expects bfloat16
            param_data = param.data.bfloat16()
            refs = [
...
def test_distributed_ray_actors(model_name="Qwen/Qwen2.5-0.5B-Instruct"):
    ...
    with start_ray_server() as address:
        if dist.get_rank() == 0:
            ...
            vllm_engines = create_vllm_engines(
                ...
                actor_class=LLMRayActorAsync,
            )

            new_port = ray.get(
                master_actor.get_free_port.remote(),  # type: ignore
            )
            print(f'new_port to init vllm process group: {new_port}')
            ...
            # otherwise it hangs
            ray.get(refs)
            print('init vllm process group done')
            ...
            ref = vllm_engines[0].generate.remote(encoded_prompts, sampling_params)  # type: ignore
            gen_results = ray.get(ref)  # type: ignore
            
            for i, (prompt, output) in enumerate(zip(prompts, gen_results)):
                if output and output.outputs:
                    ...
                    logger.warning(f'No output generated for prompt: {prompt!r}')


if __name__ == "__main__":
    test_distributed_ray_actors()