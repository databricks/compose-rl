...
def grpo_loss(
    ...
    # advantages: [bs]
    # grpo_rewards: [bs]
    # rewards: [bs, gen_len] reward for each action
    # log_probs: [bs, gen_len] log probability of each action
    # action_mask: [bs, gen_len] action mask

    advantages = batch['advantages']
    advantages = advantages.detach() # detach the advantages to avoid backpropagating
    # Check if any advantage is nan
    if torch.isnan(advantages).any():
        raise ValueError(f"Advantages contain NaN values. {advantages=}")

    online_log_probs, old_log_probs = outputs['online_log_probs'], batch['old_log_probs']
    # print(f"Inside GRPO loss, {online_log_probs.shape=}, {old_log_probs.shape=}")
    ...
    ratio = torch.exp(online_log_probs - old_log_probs)
    # print(f"Inside GRPO loss, {ratio.shape=}") # ratio is [device_train_batch_size, gen_len] 
