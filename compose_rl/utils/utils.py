...
def concat_resolved_outputs(
    output_list: list[dict[str, Union[torch.Tensor, list[Any]]]],
    pad_token_id: int,
):
    ...
    for key in output_list[0].keys():
        ...
        raw_vals = [d[key] for d in output_list]

        if key in ['verified_answer']:
            all_resolved_outputs[key] = list(flatten(raw_vals))
            continue
        ...
        # If you're one of these keys we need to do some extra padding
        # to make sure that the tensors are all the same length
        if key in [
            'prompt',
            ...
            tensor_list = padded_tensors

        all_resolved_outputs[key] = torch.cat(tensor_list, dim=0)

    return all_resolved_outputs


def partition_global_batch(
    batch: Union[torch.Tensor, list],
    world_size: int,
    rank: int,
    device_train_batch_size: int,
):
    """Partitions a batch of data evenly across all ranks.
    ...
    Note: this function assumes that each rank has the exact same set of data.

    Args:
        batch (Union[torch.Tensor, list]): The batch of data to partition.
        world_size (int): The total number of ranks.
        rank (int): The rank of the current process.
        device_train_batch_size (int): The batch size per device.
    """
    # 1) figure out length and per-rank size
    B = batch.size(0) if isinstance(batch, torch.Tensor) else len(batch)
